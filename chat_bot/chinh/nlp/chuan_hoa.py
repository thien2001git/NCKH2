...
class NLP1:
    ...
    def nlp_str(self, document:str) -> str:

        # document = "ai ơi, kia kìa, ớ"
        ...
        f = open("chinh/nlp/txt", "r", encoding="utf-8")
        q = f.readlines()
        ...
        return document
    def nlp(self, doc) -> list:
        d = []
        for document in doc:
            document = self.nlp_str(document)
            d.append(document)
        return d

class Process:
    @staticmethod
    def to_csv(name: str, cols:Iterable):
        pass
